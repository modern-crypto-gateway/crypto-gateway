...
export interface CoingeckoConfig {
  ...
  apiKey?: string;
  plan?: "pro" | "demo";
  // Cache TTL in seconds. Default 30s — short enough that a merchant's rate
  // doesn't drift far from truth, long enough that burst invoice creation
...
export function coingeckoPriceOracle(config: CoingeckoConfig): PriceOracle {
  ...
  async function fetchSpotUsd(
    ...
    // otherwise poison the TTL window and keep serving the fallback for 30s
    // even after CoinGecko recovers.
