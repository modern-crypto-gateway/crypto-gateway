  ...
  return {
    ...
    async getUsdRates(tokens) {
      ...
      try {
        ...
        logger?.warn("coingecko getUsdRates failed; using fallback only", { error });
        recordOracleFailure({ provider: "coingecko", error, tokens });
      }
      if (tokens.length > 0 && Object.keys(live).length === 0) {
        recordOracleFailure({ provider: "coingecko", error: "returned no usable rates for the requested symbols", tokens });
      }
