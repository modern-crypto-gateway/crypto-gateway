...
describe("GET /admin/chains", () => {
  ...
  it("returns the static registry with readiness flags and tokens; hides the dev chain", async () => {
    ...
    // and nothing has been bootstrapped — wired/webhooks/bootstrapReady are
    // false. `feeWallets` is now family-scoped (any HD pool address in the
    // family backs every wired chain), so it tracks "the family has a funded
    // source" rather than per-chain registration.
