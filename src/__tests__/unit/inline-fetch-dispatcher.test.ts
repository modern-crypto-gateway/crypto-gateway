...
describe("inlineFetchDispatcher", () => {
  it("signs the body with HMAC-SHA256 over the secret and sets all expected headers", async () => {
    ...
    const payload = { event: "invoice:payment_detected", data: { foo: "bar" } };
    const secret = "shared-secret-abc";
    const result = await dispatcher.dispatch({
      url: "https://example.com/hook",
      payload,
      secret,
      idempotencyKey: "invoice:payment_detected:abc:detected"
    });
    ...
    expect(headers.get("x-webhook-idempotency-key")).toBe("invoice:payment_detected:abc:detected");
